## Hi, I'm Genesis <img src="https://media.giphy.com/media/hvRJCLFzcasrR4ia7z/giphy.gif" width="40px">  

[![Typing SVG](https://readme-typing-svg.herokuapp.com?size=24&color=1ABC9C&lines=Computer+Science+Student;Aspiring+Backend+%26+DevOps+Engineer;)](https://git.io/typing-svg)

---

### 🚀 About Me  
- 🌱 Currently learning **(AWS, DevOps, Linux, Backend Architecture)**  
- 🎓 4th Year **Computer Science Student**  
- 📊 Interested in **AI, cloud projects, startups, and automation**  
- 🌍 Based in **Pampanga, Philippines**

---

### 💼 Projects
**Uni-Finder (Thesis / Capstone Project)**

🔗 https://uni-finder.dev

An AI-powered university and academic program recommender system for Senior High students in Pampanga, Philippines.

Highlights:

🤖 NLP-based semantic matching using Sentence Transformers

📐 Recommendation scoring via cosine similarity

🧠 Hybrid filtering for accurate school & program recommendations

📍 Location-aware filtering to prioritize nearby institutions

🖥 Clean, student-friendly web interface

Tech Stack:
MERN + FastAPI
MongoDB · Express · React · Node.js · FastAPI · Python · Sentence Transformers · NLP



 **Screendoor King**

🔗 https://screendoorking.vercel.app

A business website built for a screen door company to showcase products and allow customers to book services online.

Highlights:

📦 Product and service showcase

📅 Booking system with email notifications

🎨 Responsive, modern UI

Tech Stack:
React · Tailwind CSS · Email Integration · Vercel
---

### 🛠 Tech Stack  

**Languages & Tools:**  
<p>
  <img src="https://skillicons.dev/icons?i=python,fastapi,react,js,html,css,docker,linux,aws,mongodb,git,github,vscode,tailwind" />
</p>  

---

### 🌐 Connect With Me  
<p>
  <a href="https://www.linkedin.com/in/genesis-grant-vivero-861519364/" target="_blank"><img src="https://skillicons.dev/icons?i=linkedin" /></a>
  <a href="mailto:genesisgrantvivero@gmail.com"><img src="https://skillicons.dev/icons?i=gmail" /></a>
</p>  

---

⭐️ _“continuous learning.”_  
